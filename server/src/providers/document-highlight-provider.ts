...
export default class DocumentHighlightProvider implements Provider {
	...
	async onDocumentHighlight({
		textDocument,
		position
	}: lsp.DocumentHighlightParams): Promise<lsp.DocumentHighlight[] | undefined> {
		...
		if (refs) {
			for (const ref of refs) {
				results.push(
					lsp.DocumentHighlight.create(ref.location.range, lsp.DocumentHighlightKind.Read)
				);
			}
		}
		...
		if (def) {
			results.push(
				lsp.DocumentHighlight.create(def.selectionRange, lsp.DocumentHighlightKind.Write)
			);
		}
